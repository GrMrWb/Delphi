...
def feed_server_with_data_CIFAR10(val_size, train_batch_size, config, adversarial_clients, CML=False):
    if not CML:
    ...
    trainset = datasets.CIFAR10(config["collection"]["path"]["training"], download=True, train=True, transform=transform_train)
    testset= datasets.CIFAR10(config["collection"]["path"]["testing"], download=True, train=False, transform=transform_train)
