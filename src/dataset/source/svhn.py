...
def feed_server_with_data_SVHN(val_size, train_batch_size, config, adversarial_clients, CML=False):
    ...
    trainset = datasets.SVHN(config["collection"]["path"]["training"], download=True, split="train", transform=transform_train)
    testset= datasets.SVHN(config["collection"]["path"]["testing"], download=True, split="test", transform=transform_train)
