...
from src.learning.utils import pFedMeOptimizer

class ClientPFedME(ClientBase):
    def __init__(self, client_id, device, **kwargs):
        ...
        self.labda = self.server_config["learning_config"]["aggregation"][self.server_config["learning_config"]["aggregator"]]["labda"]
        self.mu = self.server_config["learning_config"]["aggregation"][self.server_config["learning_config"]["aggregator"]]["mu"]
        
        self.learning_rate = 0.01
        
        self.optimizer_per = pFedMeOptimizer(self.personalised_model.parameters(), lr=self.learning_rate, lamda=self.labda,mu=self.mu)
    ...
    def continue_training(self, dataset, epochs, **kwargs):
        ...
        for new_param, localweight in zip(self.personalised_model.parameters(), self.model.parameters()):
            localweight.data = localweight.data - self.labda * self.learning_rate * (localweight.data - new_param.data)
